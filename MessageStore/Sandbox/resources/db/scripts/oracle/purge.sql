/*
* Script to purge the Message store data.
...
BEGIN
-----------------------------------------------------PURGE MESSAGE TAGS-----------------------------------------------------------
	--Purge message entries from MS_MSG_TAG
	DELETE FROM MS_MSG_TAG WHERE MSG_HDR_ID IN (SELECT MSG_HDR_ID FROM MS_MSG_HDR
